...
function rcube_webmail() {
  ...
  this.add_label = function (p, value) {
    if (typeof p == "string") this.labels[p] = value;
  ...
  this.register_button = function (command, id, type, act, sel, over) {
    ...
    if (this.loaded) init_button(command, button_prop);
  };
  ...
  this.init = function () {
    ...
    switch (this.task) {
              ...
              if (n++)
                try {
                  if (this.contentWindow.document)
                    ref.enable_command("print", true);
                } catch (e) {
                  /* ignore */
                }
            });
          } else this.enable_command("print", true);
          ...
          if (this.env.mimetype.startsWith("image/"))
            $(this.gui_objects.messagepartframe).on("load", function () {
              ...
              $(this)
                .contents()
                .find("head")
                .append('<style type="text/css">' + css + "</style>");
            });
        }
        // show printing dialog
        else if (
          this.env.action == "print" &&
          this.env.uid &&
            ...
            .addEventListener("keypress", function (o) {
              if (o.key_pressed == o.ENTER_KEY) {
                if (!ref.compose_add_recipient()) {
                  ...
                  if (
                    o.last_selected &&
                    String(o.last_selected).charAt(0) == "G"
                  ) {
      ...
      case "addressbook":
        ...
        if (this.gui_objects.editform) {
          ...
          if (
            this.env.action == "add" ||
            this.env.action == "edit" ||
            this.env.action == "search"
          )
      ...
      case "settings":
        ...
        if (this.env.action == "identities") {
          ...
          this.env.action == "edit-identity" ||
          this.env.action == "add-identity"
        ) {
        ...
        if (this.gui_objects.identitieslist) {
          ...
          this.responses_list
            .addEventListener("select", function (list) {
              var win,
                id = list.get_single_selection();
              ref.enable_command(
                "delete",
                !!id && $.inArray(id, ref.env.readonly_responses) < 0
              );
              if (id && (win = ref.get_frame_window(ref.env.contentframe))) {
                ref.set_busy(true);
                ref.location_href(
                  { _action: "edit-response", _key: id, _framed: 1 },
                  win
                );
              }
            })
      ...
      case "login":
        ...
        if (input_user.val() == "") input_user.focus();
        else $("#rcmloginpwd").focus();

        // detect client timezone
        if (jstz && (tz = jstz.determine())) tz_name = tz.name();

        input_tz.val(
          tz_name ? tz_name : new Date().getStdTimezoneOffset() / -60
        );
    ...
    for (n in this.onloads) {
      if (typeof this.onloads[n] === "string") eval(this.onloads[n]);
      else if (typeof this.onloads[n] === "function") this.onloads[n]();
    }
  ...
  this.command = function (command, props, obj, event) {
    ...
    if (
      this.busy &&
      !(command == "reset-search" && this.last_command == "search") &&
      !command.match(/^menu-/)
    )
      return false;
    ...
    if (
      obj &&
      obj.href &&
      String(obj.href).indexOf("#") < 0 &&
      rcube_event.get_modifier(event)
    ) {
      return true;
    }
    ...
    if (
      ...
      if (
        ...
        !confirm(this.get_label("notsentwarning"))
      )
        return false;
    ...
    switch (command) {
      case "login":
        if (this.gui_objects.loginform) this.gui_objects.loginform.submit();
        break;
      ...
      case "permaurl":
        ...
        else if (this.env.permaurl) parent.location.href = this.env.permaurl;
        break;
      ...
      case "menu-open":
        if (props && props.menu == "attachmentmenu") {
          ...
          this.enable_command(
            ...
            mimetype &&
              this.env.mimetypes &&
              $.inArray(mimetype, this.env.mimetypes) >= 0
          );
        }
      ...
      case "sort":
        ...
        if (!this.env.disabled_sort_order)
          sort_order =
            this.env.sort_col == sort_col && sort_order == "ASC"
              ? "DESC"
              : "ASC";
      ...
      case "edit":
        if (this.task == "addressbook" && (cid = this.get_single_cid()))
          ...
          url[
            this.env.mailbox == this.env.drafts_mailbox && props != "new"
              ? "_draft_uid"
              : "_uid"
          ] = uid;
      ...
      case "save":
        var input,
          form = this.gui_objects.editform;
        if (form) {
          // adv. search
          ...
          // user prefs
            ...
            (input = $("input[name='_pagesize']", form)) &&
            input.length &&
          ...
          // contacts/identities
            ...
            if (props == "reload") {
              ...
              (input = $("input[name='_email']", form)) &&
              input.length &&
              !rcube_check_email(input.val())
            ) {
      ...
      case "moveto": // deprecated
        if (this.task == "mail") this.move_messages(props, event);
        else if (this.task == "addressbook") this.move_contacts(props);
        break;

      case "copy":
        if (this.task == "mail") this.copy_messages(props, event);
        else if (this.task == "addressbook") this.copy_contacts(props);
        break;
      ...
      case "toggle_flag":
        ...
        if ((uid = props)) {
          ...
          // toggle read/unread
          else if (this.message_list.rows[uid].deleted) flag = "undelete";
          else if (!this.message_list.rows[uid].unread) flag = "unread";
      ...
      case "download-attachment":
        ...
        if (
          ...
          if (
            this.open_window(this.url("get", $.extend({ _frame: 1 }, params)))
          )
            break;
        }
      ...
      case "select-all":
        ...
        if (props == "invert") this.message_list.invert_selection();
        else this.message_list.select_all(props == "page" ? "" : props);
        ...
        // modify url if we're in addressbook
          ...
          if (props && props.indexOf("@") > 0) {
            ...
            if (a_cids.length)
              this.http_post(
                "mailto",
                { _cid: a_cids.join(","), _source: this.env.source },
                true
              );
      ...
      case "send":
        if (
          !props.nocheck &&
          !this.env.is_sent &&
          !this.check_compose_input(command)
        )
          break;
      ...
      case "send-attachment":
        ...
        if (
          !(flag = this.upload_file(
            props || this.gui_objects.uploadform,
            "upload"
          ))
        ) {
          if (flag !== false) alert(this.get_label("selectimportfile"));
          aborted = true;
        }
        break;
      ...
      case "reply":
        if ((uid = this.get_single_uid())) {
          ...
          if (command == "reply-all")
            ...
            url._all =
              !props &&
              this.env.reply_all_mode == 1 &&
              this.commands["reply-list"]
                ? "list"
                : "all";
          else if (command == "reply-list") url._all = "list";
      ...
      case "forward":
        ...
        if (uids.length) {
          ...
          if (
            command == "forward-attachment" ||
            (!props && this.env.forward_attachment) ||
            uids.length > 1
          )
            url._attachment = 1;
          this.open_compose_step(url);
      ...
      case "viewsource":
        if ((uid = this.get_single_uid()))
          this.open_window(
            this.url("viewsource", this.params_from_uid(uid)),
            true,
            true
          );
        break;
      ...
      case "download":
        if (this.env.action == "get") {
          ...
          this.goto_url(
            "viewsource",
            this.params_from_uid(uid, { _save: 1 }),
            false,
            true
          );
        }
      ...
      case "reset-search":
        var n,
          s = this.env.search_request || this.env.qsearch;
        ...
        if (this.env.address_group_stack.length) {
          ...
          if (old.search_request) {
            ...
            this.list_contacts_remote(
              null,
              null,
              (this.env.current_page = old.page)
            );
          } else
            this.list_contacts(
              props.source,
      ...
      case "import":
        if (this.env.action == "import" && this.gui_objects.importform) {
          ...
          this.goto_url(
            "import",
            this.env.source ? "_target=" + urlencode(this.env.source) + "&" : ""
          );
        break;
  ...
  this.get_label = function (name, domain) {
    ...
    else if (this.labels[name]) return this.labels[name];
    else return name;
  };
  ...
  this.get_task_url = function (task, url) {
    ...
    else return url.replace(/\?.*$/, "") + "?_task=" + task;
  };

  this.reload = function (delay) {
    if (this.is_framed()) parent.rcmail.reload(delay);
    else if (delay)
      setTimeout(function () {
        ref.reload();
      }, delay);
  ...
  (this.secure_url = function (url) {
    ...
    (this.is_framed = function () {
      return (
        ...
        parent.rcmail &&
        parent.rcmail != this &&
        typeof parent.rcmail.command == "function"
      );
    });
  ...
  this.html_identifier = function (str, encode) {
    return encode
      ? this.html_identifier_encode(str)
      : String(str).replace(this.identifier_expr, "_");
  };
  ...
  this.drag_end = function (e) {
    ...
    if ((list = this.message_list)) model = this.env.mailboxes;
    else if ((list = this.contact_list)) model = this.env.contactfolders;
  ...
  this.drag_move = function (e) {
    if (this.gui_objects.folderlist) {
      ...
      if (
        this.treelist &&
        (drag_target = this.treelist.intersects(mouse, true))
      ) {
        this.env.last_folder_target = drag_target;
        layerclass =
          "draglayer" +
          (this.check_droptarget(drag_target) > 1 ? "copy" : "normal");
      } else {
  ...
  this.folder_collapsed = function (node) {
    ...
    if (node.collapsed) {
      ...
      if (
        !node.virtual &&
        this.env.mailbox &&
        this.env.mailbox.startsWith(node.id + this.env.delimiter)
      )
  ...
  this.doc_mouse_up = function (e) {
    var list,
      id,
      target = rcube_event.get_target(e);
      ...
      function (e) {
        ...
        for (i = ref.menu_stack.length - 1; i >= 0; i--) {
          ...
          if (
            ...
            id != skip &&
            (obj.attr("data-editable") != "true" ||
              !$(target).parents("#" + id).length) &&
            (obj.attr("data-sticky") != "true" ||
              !rcube_mouse_is_over(e, obj.get(0)))
          ) {
            ref.hide_menu(id, e);
          }
    ...
    var focus_menu_item = function (dir) {
      ...
      if (ref.focused_menu && (obj = $("#" + ref.focused_menu))) {
        item = obj
          .find(":focus")
          .closest("li")
          [mod](":has(:not([aria-disabled=true]))")
          .find("a,input")
          [limit]();
        if (!item.length)
          item = obj
  ...
  this.msglist_select = function (list) {
    ...
    this.enable_command(
      ...
      "forward",
      "forward-attachment",
      list.selection.length > 0
    );

    // reset all-pages-selection
    if (
      selected ||
      (list.selection.length && list.selection.length != list.rowcount)
    )
      this.select_all_mode = false;
  ...
  this.msglist_dbl_click = function (list) {
    ...
    if (
      uid &&
      (this.env.messages[uid].mbox || this.env.mailbox) ==
        this.env.drafts_mailbox
    )
      this.open_compose_step({ _draft_uid: uid, _mbox: this.env.mailbox });
    else if (uid) this.show_message(uid, false, false);
  };

  this.msglist_keypress = function (list) {
    if (list.modkey == CONTROL_KEY) return;

    if (list.key_pressed == list.ENTER_KEY) this.command("show");
    else if (
      list.key_pressed == list.DELETE_KEY ||
      list.key_pressed == list.BACKSPACE_KEY
    )
      this.command("delete");
    else if (list.key_pressed == 33) this.command("previouspage");
    else if (list.key_pressed == 34) this.command("nextpage");
  };
  ...
  this.msglist_get_preview = function () {
    ...
    else if (this.env.contentframe) this.show_contentframe(false);
  };
    ...
    switch (this.task) {
      case "mail":
        return this.env.mailboxes[id] &&
          ...
          (this.env.mailboxes[id].id != this.env.mailbox ||
      ...
      case "addressbook":
        ...
        if (id != this.env.source && (target = this.env.contactfolders[id])) {
          ...
          if (target.type == "group") {
            if (
              target.id != this.env.group &&
              !this.env.contactfolders[target.source].readonly
            ) {
              var is_other =
                this.env.selection_sources.length > 1 ||
                $.inArray(target.source, this.env.selection_sources) == -1;
          ...
          // droptarget is a (writable) addressbook and it's not the source
            ...
            !target.readonly &&
            (this.env.selection_sources.length > 1 ||
              $.inArray(id, this.env.selection_sources) == -1)
          ) {
  ...
  this.open_window = function (url, small, toolbar) {
    ...
    if (this.env.standard_windows) var extwin = window.open(url, wname);
    else {
      var win = this.is_framed() ? parent.window : window,
        ...
        w = Math.min(
          small ? this.env.popup_width_small : this.env.popup_width,
          page_width
        ),
  ...
  this.init_message_row = function (row) {
    var i,
      ...
      uid = row.uid,
      status_icon =
        (this.env.status_col != null ? "status" : "msg") + "icn" + row.id;
    ...
    else row.msgicon = row.icon;

    // set eventhandler to flag icon
    if (
      this.env.flagged_col != null &&
      (row.flagicon = document.getElementById("flagicn" + row.id))
    ) {
    ...
    if (
      !row.depth &&
      row.has_children &&
      (row.expando = document.getElementById("rcmexpando" + row.id))
    ) {
  ...
  this.add_message_row = function (uid, cols, flags, attop) {
    ...
    if (this.env.threading) {
      if (message.depth) {
        ...
        if (
          (rows[message.parent_uid] &&
            rows[message.parent_uid].expanded === false) ||
          ((this.env.autoexpand_threads == 0 ||
            this.env.autoexpand_threads == 2) &&
        ...
        if (
          ...
          (this.env.autoexpand_threads == 1 ||
            (this.env.autoexpand_threads == 2 && message.unread_children))
        ) {
    ...
    if (cols.subject) {
      var action = flags.mbox == this.env.drafts_mailbox ? "compose" : "show",
        uid_param =
          flags.mbox == this.env.drafts_mailbox ? "_draft_uid" : "_uid",
    ...
    for (n in this.env.listcols) {
      ...
      if (c == "flag") {
        ...
        if (flags.attachmentClass)
          ...
          flags.ctype == "multipart/encrypted" ||
          flags.ctype == "application/pkcs7-mime"
        )
        ...
        html =
          ...
          label +
          '"></span>';
      } else if (c == "threads") html = expando;
      else if (c == "subject") {
        html = tree + cols[c];
      } else if (c == "priority") {
  ...
  this.widescreen_message_row = function (row, uid, message) {
    ...
    if (this.env.threading && message.depth) {
      $("td.subject", domrow).attr(
        "style",
        "padding-left:" + Math.min(90, message.depth * 15) + "px !important"
      );
  ...
  this.set_list_sorting = function (sort_col, sort_order) {
    ...
    $("#rcm" + sort_old).removeClass(
      "sorted" + this.env.sort_order.toUpperCase()
    );
  ...
  this.set_list_options = function (
    cols,
    sort_col,
    sort_order,
    threads,
  ...
  this.show_message = function (id, safe, preview) {
    ...
    if (preview && String(target.location.href).indexOf(url) >= 0) {
      ...
      else this.location_href(url, target, true);
    }
  };
  ...
  this.set_unread_message = function (id, folder) {
    ...
    if (self.env.unread_counts[folder] > 0) {
      ...
      self.set_unread_count(
        folder,
        self.env.unread_counts[folder],
        folder == "INBOX" && !self.is_multifolder_listing()
      );
    }
  };
    ...
    if ((frame = this.get_frame_element(name))) {
      if (!show && (win = this.get_frame_window(name))) {
        if (win.location.href.indexOf(this.env.blankpage) < 0) {
          ...
          // IE
          else win.document.execCommand("Stop");
  ...
  this.lock_frame = function () {
    if (!this.env.frame_lock)
      (this.is_framed() ? parent.rcmail : this).env.frame_lock = this.set_busy(
        true,
        "loading"
      );
  };
  ...
  this.list_page = function (page) {
    if (page == "next") page = this.env.current_page + 1;
    else if (page == "last") page = this.env.pagecount;
    else if (page == "prev" && this.env.current_page > 1)
      page = this.env.current_page - 1;
    else if (page == "first" && this.env.current_page > 1) page = 1;
    ...
    if (page > 0 && page <= this.env.pagecount) {
      ...
      else if (this.task == "mail") this.list_mailbox(this.env.mailbox, page);
    }
  };
  ...
  this.list_mailbox = function (mbox, page, sort, url, update_only) {
    ...
    if (!update_only) {
      ...
      if (
        ...
        (mbox == this.env.mailbox && !page && !sort)
      )
        url._refresh = 1;
  ...
  this.update_selection = function () {
    ...
    for (i in selected) if (rows[selected[i]]) selection.push(selected[i]);
  ...
  this.expand_unread = function () {
    ...
    while (new_row) {
      if (
        new_row.nodeType == 1 &&
        (r = this.message_list.rows[new_row.uid]) &&
        r.unread_children
      ) {
  ...
  this.add_tree_icons = function (root) {
    ...
    if (root) row = rows[root] ? rows[root].obj : null;
    else row = this.message_list.tbody.firstChild;
  ...
  this.set_tree_icons = function (uid, tree) {
    ...
    for (i = 0; i < len; i++) {
      if (tree[i] > 2) divs.push({ class: "l3", width: 15 });
      else if (tree[i] > 1) divs.push({ class: "l2", width: 15 });
      else if (tree[i] > 0) divs.push({ class: "l1", width: 15 });
      // separator div
    ...
    for (i = divs.length - 1; i >= 0; i--) {
      ...
      else html += '<div style="width:' + divs[i].width + 'px" />';
    }
  ...
  this.update_thread = function (uid) {
    ...
    if (!row.depth)
      // root message: decrease roots count
      count--;
        ...
        if (!r.depth) {
          ...
          if (r.has_children) {
            ...
            $("#" + r.id + " .leaf:first")
              ...
              .attr(
                "class",
                r.obj.style.display != "none" ? "expanded" : "collapsed"
              )
  ...
  this.copy_messages = function (mbox, event) {
    if (mbox && typeof mbox === "object") mbox = mbox.id;
    else if (!mbox)
    ...
    this.http_post(
      "copy",
      post_data,
      this.display_message(this.get_label("copyingmessage"), "loading")
    );
  };
  ...
  this.move_messages = function (mbox, event) {
    ...
    var lock = false,
      post_data = this.selection_post_data({ _target_mbox: mbox });
  ...
  this.delete_messages = function (event) {
    ...
    // if there is a trash mailbox defined and we're not currently in it
      ...
      if (
        (list && list.modkey == SHIFT_KEY) ||
        (event && rcube_event.get_modifier(event) == SHIFT_KEY)
      ) {
        if (confirm(this.get_label("deletemessagesconfirm")))
          this.permanently_remove_messages();
      } else this.move_messages(trash);
    }
  ...
  this.with_selected_messages = function (
    action,
    post_data,
    lock,
    http_action
    ...
    if (this.message_list) {
      ...
      for (n = 0, len = selection.length; n < len; n++) {
        ...
        if (remove)
          this.message_list.remove_row(
            id,
            this.env.display_next && n == selection.length - 1
          );
      }
      // make sure there are no selected rows
      if (!this.env.display_next && remove) this.message_list.clear_selection();
  ...
  this.mark_message = function (flag, uid) {
    ...
    if (uid) a_uids[0] = uid;
    else if (this.env.uid) a_uids[0] = this.env.uid;
    else if (list) a_uids = list.get_selection();

    if (!list) r_uids = a_uids;
    else {
      list.focus();
      for (n = 0, len = a_uids.length; n < len; n++) {
        id = a_uids[n];
        if (
          (flag == "read" && list.rows[id].unread) ||
  ...
  this.toggle_delete_status = function (a_uids) {
    ...
    if (len == 1) {
      ...
      else this.flag_as_undeleted(a_uids);
    ...
    if (all_deleted) this.flag_as_undeleted(a_uids);
    else this.flag_as_deleted(a_uids);
  ...
  this.flag_as_deleted = function (a_uids) {
    ...
    for (var i = 0, len = a_uids.length; i < len; i++) {
      ...
      if (rows[uid]) {
        ...
        if (this.env.skip_deleted) {
          ...
          list.remove_row(
            uid,
            this.env.display_next && i == list.selection.length - 1
          );
    ...
    if (this.env.skip_deleted && list) {
      if (!this.env.display_next || !list.rowcount) list.clear_selection();
      if (count < 0) post_data._count = count * -1;
      else if (count > 0)
        // remove threads from the end of the list
      ...
      .on("keydown keyup click", function (e) {
        ...
        if ((current = $(list[p - 1])).length) {
          ...
          $("#pagejump-selector").scrollTop(
            (ul.height() / list.length) * (p - 1) - selector.height() / 2
          );
        }
      })
  ...
  this.mailvelope_load = function (action) {
    ...
    mailvelope.getKeyring(keyring).then(
      function (kr) {
      ...
      function (err) {
        ...
        mailvelope.createKeyring(keyring).then(
          function (kr) {
            ref.mailvelope_keyring = kr;
            ref.mailvelope_init(action, kr);
          },
  ...
  this.mailvelope_init = function (action, keyring) {
    ...
    if (action == "show" || action == "preview" || action == "print") {
      ...
      if (this.env.is_pgp_content) {
        ...
        ref.mailvelope_display_container(
          this.env.is_pgp_content,
          data,
          keyring
        );
      ...
      // load pgp/mime message and pass it to the mailvelope display container
        ...
        var msgid = this.display_message(
            this.get_label("loadingdata"),
            "loading"
          ),
  ...
  this.compose_encrypted = function (props) {
    var options,
      container = $("#" + this.env.composebody).parent();
  ...
  this.mailvelope_submit_messageform = function (draft, saveonly) {
    ...
    ref.mailvelope_keyring.validKeyForAddress(recipients).then(
      function (status) {
        var missing_keys = [];
        ...
        if (!isvalid && missing_keys.length) {
          ...
          ref.simple_dialog(
            ref
              .get_label("nopubkeyfor")
              .replace("$email", missing_keys.join(", ")) +
              "<p>" +
        ...
        var senders = [],
          selected_sender =
            ref.env.identities[$("[name='_from'] option:selected").val()];
        $.each(ref.env.identities, function (k, sender) {
          senders.push(sender.email);
        });

        ref.mailvelope_keyring.validKeyForAddress(senders).then(
          function (status) {
            valid_sender = null;
            $.each(status, function (k, v) {
              if (v !== false) {
            ...
            ref.mailvelope_editor.encrypt(recipients).then(
              function (armored) {
                ...
                var form = ref.gui_objects.messageform,
                  ...
                  msgid = ref.set_busy(
                    true,
                    draft || saveonly ? "savingmessage" : "sendingmessage"
                  );
                ...
                if (!hidden.length) {
                  hidden = $('<input type="hidden" name="_pgpmime">').appendTo(
                    form
                  );
                }
                hidden.val(armored);
  ...
  this.mailvelope_display_container = function (
    selector,
    data,
    keyring,
    msgid
    ...
    mailvelope
      .createDisplayContainer(selector, data, keyring, {
  ...
  this.mailvelope_key_import_dialog = function (candidates, import_handler) {
    ...
    $.each(candidates, function (i, keyrec) {
      ...
      li.append(
        $("<a>")
          .text(keyrec.keyid.substr(-8).toUpperCase())
      ...
      if (keyrec.expirationdate) {
        li.append(
          $("<label>").addClass("keyexpired").text(ref.get_label("keyexpired"))
        );
        li.append(
          $("<span>").text(
            new Date(keyrec.expirationdate * 1000).toDateString()
          )
        );
      }
      ...
      if (keyrec.revoked) {
        li.append(
          $("<span>").addClass("keyrevoked").text(ref.get_label("keyrevoked"))
        );
      }
    ...
    ul.on("click", "input.button.importkey", function () {
      ...
      pk.get(keyid, function (armored, errorCode) {
        ...
        // import to keyring
        ref.mailvelope_keyring.importPublicKey(armored).then(
          function (status) {
            if (status === "REJECTED") {
              ...
              ref.display_message(
                ref.get_label("keyimportsuccess").replace("$key", $key),
                "confirmation"
              );
            }
          },
  ...
  this.purge_mailbox_test = function () {
    return (
      ...
      (this.env.mailbox == this.env.trash_mailbox ||
        this.env.mailbox == this.env.junk_mailbox ||
        this.env.mailbox.startsWith(
          this.env.trash_mailbox + this.env.delimiter
        ) ||
  ...
  this.mark_all_read = function (mbox, mode) {
    ...
    this.http_post(
      "mark",
      post_data,
      this.display_message(this.get_label("markingmessage"), "loading")
    );
  };
  ...
  this.init_messageform = function () {
    ...
    if (opener_rc && opener_rc.env.action == "compose") {
      setTimeout(function () {
        if (opener.history.length > 1) opener.history.back();
        else opener_rc.redirect(opener_rc.get_task_url("mail"));
      }, 100);
    ...
    for (i in ac_fields) {
      this.init_address_input_events(
        $("[name='_" + ac_fields[i] + "']"),
        ac_props
      );
    }
    ...
    if (!html_mode) {
      pos =
        this.env.top_posting && this.env.compose_mode
          ? 0
          : input_message.value.length;
    ...
    if (input_to.val() == "") elem = input_to;
    else if (input_subject.val() == "") elem = input_subject;
    else if (input_message) elem = input_message;
  ...
  this.compose_restore_dialog = function (j, html_mode) {
    var i,
      key,
      formdata,
      index = this.local_storage_get_item("compose.index", []);
    ...
    if (this.contact_list && this.contact_list.selection.length) {
      for (var id, n = 0; n < this.contact_list.selection.length; n++) {
        ...
        if (id && this.env.contactdata[id]) {
          ...
          if (
            id.charAt(0) == "E" &&
            this.env.contactdata[id].indexOf("@") < 0 &&
            input.length
          ) {
            var gid = id.substr(1);
            ...
            this.http_request(
              "group-expand",
              { _source: this.env.source, _gid: gid },
              false
            );
          }
        }
  ...
  this.check_compose_input = function (cmd) {
    ...
    if (
      input_from.prop("type") == "text" &&
      !rcube_check_email(input_from.val(), true)
    ) {
    ...
    if (
      !rcube_check_email(get_recipients([input_to, input_cc, input_bcc]), true)
    ) {
    ...
    for (key in this.env.attachments) {
      if (
        typeof this.env.attachments[key] === "object" &&
        !this.env.attachments[key].complete
      ) {
    ...
    if (
      ...
      rcube_check_email(
        (recipients = get_recipients([input_to, input_cc])),
        true,
        true
      ) > limit
    ) {
    ...
    if (!this.env.nosubject_warned && input_subject.val() == "") {
      var prompt_value = $("<input>").attr({ type: "text", size: 40 }),
        myprompt = $('<div class="prompt">')
          .append(
            $('<div class="message">').text(this.get_label("nosubjectwarning"))
          )
          .append(prompt_value),
        save_func = function () {
  ...
  this.save_response = function () {
    ...
    buttons[this.get_label("save")] = function (e) {
      ...
      var lock = ref.display_message(
        ref.get_label("savingresponse"),
        "loading"
      );
      ref.http_post(
        "settings/responses",
        { _insert: 1, _name: name, _text: text },
        lock
      );
      $(this).dialog("close");
    };
  ...
  this.set_draft_id = function (id) {
    if (id && id != this.env.draft_id) {
      ...
      if (
        window.frames["savetarget"] &&
        window.frames["savetarget"].history &&
        !this.draft_autosave_submit &&
        !this.mailvelope_editor
      ) {
  ...
  this.compose_field_hash = function (save) {
    ...
    if (this.env.attachments) for (id in this.env.attachments) str += id;
  ...
  this.save_compose_form_local = function () {
    ...
    $("input, select, textarea", this.gui_objects.messageform).each(function (
      i,
      elem
    ) {
      switch (elem.tagName.toLowerCase()) {
        case "input":
          if (
            elem.type == "button" ||
            elem.type == "submit" ||
            (elem.type == "hidden" && elem.name != "_is_html")
          ) {
            break;
          }
          formdata[elem.name] =
  ...
  this.restore_compose_form = function (key, html_mode) {
    var ed,
      formdata = this.local_storage_get_item("compose." + key, true);
  ...
  this.change_identity = function (obj, show_sig) {
    ...
    if (!show_sig) show_sig = this.env.show_sig;
    ...
    $.each(["replyto", "bcc"], function () {
      var rx,
        key = this,
        old_val =
          sig && ref.env.identities[sig] ? ref.env.identities[sig][key] : "",
        new_val =
          id && ref.env.identities[id] ? ref.env.identities[id][key] : "",
        input = $('[name="_' + key + '"]'),
        input_val = input.val();
      ...
      if (
        new_val &&
        input_val.indexOf(new_val) == -1 &&
        input_val.indexOf(new_val.replace(/"/g, "")) == -1
      ) {
        if (input_val) {
  ...
  this.upload_file = function (form, action, lock) {
    ...
    $.each($(form).get(0).elements || [], function () {
      ...
      var i,
        files = this.files ? this.files.length : this.value ? 1 : 0;

      // check file size
      if (this.files) {
        for (i = 0; i < files; i++) size += this.files[i].size;
      }
    ...
    if (numfiles) {
      ...
      if (
        ...
        numfiles > this.env.max_filecount
      ) {
        this.display_message(this.env.filecounterror, "error");
        return false;
      }

      var frame_name = this.async_upload_form(
        form,
        action || "upload",
        function (e) {
          var d,
          ...
          if (
            !content.match(/add2attachment/) &&
            (!bw.opera ||
              (ref.env.uploadframe && ref.env.uploadframe == e.data.ts))
          ) {
            if (!content.match(/display_message/))
              ref.display_message(ref.get_label("fileuploaderror"), "error");
            ref.remove_from_attachment_list(e.data.ts);

            if (lock) ref.set_busy(false, null, lock);
          }
          // Opera hack: handle double onload
          if (bw.opera) ref.env.uploadframe = e.data.ts;
        }
      );
      ...
      var content =
          "<span>" +
          this.get_label("uploading" + (numfiles > 1 ? "many" : "")) +
          "</span>",
  ...
  this.add2attachment_list = function (name, att, upload_id) {
    ...
    if (!this.env.attachments) this.env.attachments = {};
    ...
    var tabindex =
      $(this.gui_objects.attachmentlist).attr("data-tabindex") || "0";
  ...
  this.cancel_attachment_upload = function (name, frame_name) {
    if (!name || !frame_name) return false;
  ...
  this.upload_progress_update = function (param) {
    ...
    if (!elem.length || !param.text) return;

    elem.text(param.text);

    if (!param.done) this.upload_progress_start(param.action, param.name);
  };
    ...
    this.simple_dialog(
      ...
      function () {
        ...
        if ((name = input.val()) && name != attachment.name) {
          ref.http_post(
            "rename-attachment",
            { _id: ref.env.compose_id, _file: id, _name: name },
  ...
  this.add_contact = function (value) {
    if (value) this.http_post("addcontact", { _address: value });
  ...
  this.qsearch = function (value) {
    ...
    if (
      ...
      var r,
        ...
        action =
          this.env.action == "compose" && this.contact_list
            ? "search-contacts"
            : "search";

      if (this.message_list) this.clear_message_list();
      else if (this.contact_list) this.list_contacts_clear();
      ...
      this.enable_command(
        "set-listmode",
        this.env.threads && (this.env.search_scope || "base") == "base"
      );
  ...
  this.search_params = function (search, filter) {
    ...
    if (scope) url._scope = scope;
    if (mbox && scope != "all") url._mbox = mbox;
  ...
  this.set_searchscope = function (scope) {
    ...
    if (scope != old && this.env.search_request) {
      if (
        ...
        this.env.search_filter != "ALL"
      )
        this.filter_mailbox(this.env.search_filter);
      if (scope != "all") this.select_folder(this.env.mailbox, "", true);
    }
  };
  ...
  this.set_searchinterval = function (interval) {
    ...
    if (interval != old && this.env.search_request) {
      if (
        ...
        this.filter_mailbox(this.env.search_filter);
      if (interval) this.select_folder(this.env.mailbox, "", true);
    }
  };
  ...
  this.is_multifolder_listing = function () {
    return this.env.multifolder_listing !== undefined
  ...
  this.ksearch_keydown = function (e, obj, props) {
    ...
    switch (key) {
      ...
      case 40: // arrow down
        ...
        var dir = key == 38 ? 1 : 0,
          highlight = document.getElementById(
        ...
        if (highlight)
          this.ksearch_select(
            dir ? highlight.previousSibling : highlight.nextSibling
          );
  ...
  this.ksearch_visible = function () {
    return (
      ...
      this.ksearch_selected !== undefined &&
      this.ksearch_value
    );
  };
  ...
  this.insert_recipient = function (id) {
    ...
    var inp_value = this.ksearch_input.value,
      ...
      end = inp_value.substring(
        p + this.ksearch_value.length,
        inp_value.length
      );
    ...
    if (
      typeof this.env.contacts[id] === "object" &&
      this.env.contacts[id].type == "group" &&
      !this.env.contacts[id].email
    ) {
  ...
  this.replace_group_recipients = function (id, recipients) {
    if (this.group2expand[id]) {
      this.group2expand[id].input.value = this.group2expand[
        id
      ].input.value.replace(this.group2expand[id].name, recipients);
      this.triggerEvent("autocomplete_insert", {
  ...
  this.ksearch_query_results = function (results, search, reqid) {
    ...
    if (results && (len = results.length)) {
      for (i = 0; i < len && maxlen > 0; i++) {
        ...
        $("<li>")
          .attr("id", "rcmkSearchItem" + id)
          .attr("role", "option")
          .html(
            '<i class="icon"></i>' +
              this.quote_html(
                text.replace(
                  new RegExp("(" + RegExp.escape(value) + ")", "ig"),
                  "##$1%%"
                )
              ).replace(/##([^%]+)%%/g, "<b>$1</b>")
          )
  ...
  this.contactlist_keypress = function (list) {
    if (list.key_pressed == list.DELETE_KEY) this.command("delete");
  };
  ...
  this.contactlist_select = function (list) {
      ...
      for (n in list.selection) {
        ...
        if (!source) {
          ...
          if (sid && this.env.address_sources[sid]) {
            writable =
              writable ||
              (!this.env.address_sources[sid].readonly && !contact.readonly);
    ...
    this.enable_command(
      ...
      this.env.group && selected && writable
    );
  ...
  this.list_contacts = function (src, group, page, search) {
    ...
    if (this.env.search_id) folder = "S" + this.env.search_id;
    else if (!this.env.search_request) folder = group ? "G" + src + group : src;
    ...
    this.env.address_group_stack =
      index < 0 ? [] : this.env.address_group_stack.slice(0, index);
  ...
  this.list_contacts_remote = function (src, group, page) {
    ...
    this.http_request(
      this.env.task == "mail" ? "list-contacts" : "list",
      url,
      lock
    );
  ...
  this.set_group_prop = function (prop) {
    if (this.gui_objects.addresslist_title) {
      ...
      boxtitle.append(
        $("<span>").text(prop ? prop.name : this.get_label("contacts"))
      );
    }
  };
  ...
  this.group_member_change = function (what, cid, source, gid) {
    ...
    var label = this.get_label(
        what == "add" ? "addingmember" : "removingmember"
      ),
  ...
  this.contacts_drag_menu = function (e, to) {
    ...
    if (
      !this.env.address_sources[dest] ||
      this.env.address_sources[dest].readonly
    )
      return true;
  ...
  this.delete_contacts = function () {
    ...
    if (!undelete && !confirm(this.get_label("deletecontactconfirm"))) return;
  ...
  this._with_selected_contacts = function (action, post_data) {
    ...
    if (this.env.cid) a_cids.push(this.env.cid);
    else {
      ...
      if (selection.length == 1) this.show_contentframe(false);
    }
    ...
    this.simple_dialog(
      ...
      function () {
        ...
        if ((name = input.val()) && name != group_name) {
          ref.http_post(
            "group-rename",
            { _source: source, _gid: group, _name: name },
            ref.set_busy(true, "loading")
          );
          return true;
        }
  ...
  this.remove_group_contacts = function (props) {
    if (this.env.group !== undefined && this.env.group === props.gid) {
      var n,
        selection = this.contact_list.get_selection();
      for (n = 0; n < selection.length; n++) {
        id = selection[n];
        this.contact_list.remove_row(id, n == selection.length - 1);
      }
  ...
  this.insert_contact_group = function (prop) {
    ...
    this.treelist.insert(
      { id: key, html: link, classes: ["contactgroup"] },
      prop.source,
      "contactgroup"
    );
  ...
  this.init_edit_field = function (col, elem) {
    ...
    if (label) elem.placeholder(label);
  };
  ...
  this.insert_edit_field = function (col, section, menu) {
    ...
    if (elem.length) {
      elem.show().focus();
      $(menu)
        .children('option[value="' + col + '"]')
        .prop("disabled", true);
    } else {
      var lastelem = $(".ff_" + col),
        appendcontainer = $(
          "#contactsection" + section + " .contactcontroller" + col
        );

      if (!appendcontainer.length) {
        var sect = $("#contactsection" + section),
          lastgroup = $(".contactfieldgroup", sect).last();
        appendcontainer = $("<fieldset>").addClass(
          "contactfieldgroup contactcontroller" + col
        );
        if (lastgroup.length) appendcontainer.insertAfter(lastgroup);
        else sect.prepend(appendcontainer);
      }

      if (
        appendcontainer.length &&
        appendcontainer.get(0).nodeName == "FIELDSET"
      ) {
        var input,
        ...
        if (colprop.subtypes_select) label.html(colprop.subtypes_select);
        else
          label.html(
            '<label for="' + input_id + '">' + colprop.label + "</label>"
          );
        ...
        if (colprop.type == "text" || colprop.type == "date") {
          ...
          if (colprop.type == "date" && $.datepicker) input.datepicker();
          ...
          if ((templ = this.env[col + "_template"])) {
            ...
            // list fields according to appearance in colprop
  ...
  this.delete_edit_field = function (elem) {
    ...
    if (addmenu.length) {
      ...
      if (option.length) option.prop("disabled", false);
      else
        option = $("<option>")
          .attr("value", col)
          .html(colprop.label)
  ...
  this.set_photo_actions = function (id) {
    var n,
      buttons = this.buttons["upload-photo"];
    for (n = 0; buttons && n < buttons.length; n++)
      $("a#" + buttons[n].id).html(
        this.get_label(id == "-del-" ? "addphoto" : "replacephoto")
      );
    ...
    this.enable_command(
      "delete-photo",
      this.env.coltypes.photo && id != "-del-"
    );
  };
  ...
  this.search_create = function () {
    ...
    this.simple_dialog(content, "searchsave", function () {
      var name;
      if ((name = input.val())) {
        ref.http_post(
          "search-create",
          { _search: ref.env.search_request, _name: name },
          ref.set_busy(true, "loading")
        );
        return true;
      }
  ...
  this.identity_select = function (list) {
    ...
    if ((id = list.get_single_selection())) {
      this.enable_command(
        "delete",
        list.rowcount > 1 && this.env.identities_level < 2
      );
  ...
  this.load_identity = function (id, action) {
    if (action == "edit-identity" && (!id || id == this.env.iid)) return false;
  ...
  this.delete_identity = function (id) {
    ...
    if (!(selection.length || this.env.iid)) return;

    if (!id) id = this.env.iid ? this.env.iid : selection[0];
  ...
  this.update_identity_row = function (id, name, add) {
    ...
    if (add) {
      list.insert_row({
        id: "rcmrow" + rid,
        cols: [{ className: "mail", innerHTML: name }],
      });
      list.select(rid);
    } else {
  ...
  this.update_response_row = function (response, oldkey) {
    ...
    if (list && oldkey) {
      ...
      list.insert_row({
        ...
        cols: [{ className: "name", innerHTML: response.name }],
      });
  ...
  this.subscription_move_folder = function (from, to) {
    if (
      ...
      to !== null &&
      from != to &&
      to != from.replace(this.last_sub_rx, "")
    ) {
      var path = from.split(this.env.delimiter),
        ...
        newname =
          to === "" || to === "*"
            ? basename
            : to + this.env.delimiter + basename;

      if (newname != from) {
        this.http_post(
          "rename-folder",
          { _folder_oldname: from, _folder_newname: newname },
  ...
  this.add_folder_row = function (
    ...
    $('input[name="_subscribed[]"]:first', row)
      .val(id)
      .prop({
    ...
    folders.sort(function (a, b) {
      ...
      for (i = 0; i < len; i++) {
        ...
        if (f1 !== f2) {
          ...
          if (collator) return collator.compare(f1, f2);
  ...
  this.replace_folder_row = function (
    oldid,
    id,
    name,
    display_name,
    ...
    if (!this.gui_objects.subscriptionlist) {
      if (this.is_framed()) {
        ...
        return window.parent.rcmail.replace_folder_row(
          oldid,
          id,
          name,
          display_name,
    ...
    if (
      parent.get(0) != this.gui_objects.subscriptionlist &&
      !$("li", parent).length
    ) {
  ...
  this.subscribe = function (folder) {
    if (folder) {
      var lock = this.display_message(
        this.get_label("foldersubscribing"),
        "loading"
      );
  ...
  this.unsubscribe = function (folder) {
    if (folder) {
      var lock = this.display_message(
        this.get_label("folderunsubscribing"),
        "loading"
      );
  ...
  this.show_folder = function (folder, path, force) {
    ...
    if (String(target.location.href).indexOf(url) >= 0 && !force)
      this.show_contentframe(true);
    else this.location_href(this.env.comm_path + url, target, true);
  };
  ...
  this.disable_subscription = function (folder) {
    ...
    if (row) $('input[name="_subscribed[]"]:first', row).prop("disabled", true);
  };

  // resets state of subscription checkbox (e.g. on error)
  this.reset_subscription = function (folder, state) {
    var row = this.subscription_list.get_item(folder, true);
    if (row) $('input[name="_subscribed[]"]:first', row).prop("checked", state);
  };
  ...
  this.folder_filter = function (prefix) {
    ...
    this.subscription_list.container.children("li").each(function () {
      var i,
        folder = ref.folder_id2name(this.id);
      // show all folders
      if (prefix == "---") {
      }
    ...
    for (n = 0; n < len; n++) {
      ...
      if (button.type == "image" && !button.status) {
        ...
        if (
          ...
          obj.runtimeStyle.filter.match(/src=['"]([^'"]+)['"]/)
        )
          button.pas = RegExp.$1;
      ...
      if (button.type == "input") {
        ...
        $obj
          .attr(
            ...
            state == "pas" || state == "sel"
              ? "-1"
              : $obj.attr("data-tabindex") || "0"
          )
          .attr(
            "aria-disabled",
            state == "pas" || state == "sel" ? "true" : "false"
          );
      }
    }
  };
  ...
  this.set_alttext = function (command, label) {
    ...
    for (n = 0; n < len; n++) {
      ...
      } else if (obj) obj.setAttribute("title", this.get_label(label));
    }
  };
  ...
  this.display_message = function (msg, type, timeout, key) {
    ...
    if (this.messages[key]) {
      // replace label
      if (this.messages[key].obj) this.messages[key].obj.html(msg);
  ...
  this.clear_messages = function () {
    ...
    for (k in m)
      for (n in m[k].elements) if (m[k].obj) this.hide_message_object(m[k].obj);
  ...
  this.show_popup_dialog = function (content, title, buttons, options) {
    ...
    if (typeof content == "object") popup.append(content);
    ...
    $.each(options.button_classes || [], function (i, v) {
      if (v)
        $(
          $(".ui-dialog-buttonpane button.ui-button", popup.parent()).get(i)
        ).addClass(v);
    });
      ...
      buttons = [
        {
          ...
          class: "mainaction",
          click: function () {
            if (button_func()) $(this).dialog("close");
          },
        },
        {
  ...
  this.set_message_coltypes = function (listcols, repl, smart_col) {
    ...
    if (thead) {
      ...
      for (n = 0, len = this.env.listcols.length; n < len; n++) {
        ...
        if (
          (cell = thead.rows[0].cells[n]) &&
          (col == "from" || col == "to" || col == "fromto")
        ) {
          $(cell)
            .attr("rel", col)
            .find("span,a")
            .text(this.get_label(col == "fromto" ? smart_col : col));
        }
      }
    }
    ...
    if (list) {
      list.hide_column(
        ...
        (this.env.coltypes.folder && this.env.coltypes.folder.hidden) ||
          $.inArray("folder", this.env.listcols) < 0
      );
      list.init_header();
    }
  };
  ...
  this.set_unread_count = function (mbox, count, set_title, mark) {
    ...
    if (mark) this.mark_folder(mbox, mark, "", true);
    else if (!count) this.unmark_folder(mbox, "recent", "", true);
  ...
  this.set_unread_count_display = function (mbox, set_title) {
    ...
    if ((item = this.get_folder_li(mbox, "", true))) {
      ...
      else if (text_obj.length) text_obj.remove();
      ...
      if (mycount + childcount > 0) $(item).addClass("unread");
      else $(item).removeClass("unread");
    }
  ...
  this.folder_selector = function (event, callback) {
    ...
    if (!container) {
      ...
      $.each(this.env.mailboxes_list, function () {
        ...
        if (folder.virtual)
          a.addClass("virtual")
            ...
            .attr("tabindex", "-1");
        else a.addClass("active").data("id", folder.id);

        if (folder["class"]) a.addClass(folder["class"]);
      ...
      if (rows.length > 10)
        container.css(
          "max-height",
          $("li", container)[0].offsetHeight * 10 + 9
        );
  ...
  this.show_menu = function (prop, show, event) {
    ...
    if (show && ref.length) {
      ...
      stack =
        ref.attr("role") == "menuitem" ||
        ref.closest("[role=menuitem]").length > 0;
      ...
      pos.top = Math.max(
        0,
        pos.top + (above ? -obj.height() : ref.offsetHeight)
      );
    ...
    if (show) {
      ...
      for (var i = this.menu_stack.length - 1; stack && i >= 0; i--) {
        if (
          !$(ref).parents("#" + this.menu_stack[i]).length &&
          $(event.target).parent().attr("role") != "menuitem"
        )
      ...
      obj
        ...
        .data("opener", ref.attr("aria-expanded", "true").get(0));
  ...
  this.format_converter = function (text, format, func) {
    ...
    if (
      !text ||
      (format == "html" &&
        !text.replace(/<[^>]+>|&nbsp;|\xC2\xA0|\s/g, "").length) ||
    ...
    var url =
        "?_task=utils&_action=" +
        (format == "html" ? "html2text" : "text2html"),
  ...
  this.url = function (action, query) {
    ...
    if (typeof action !== "string") query = action;
    else if (!query || typeof query !== "object") query = {};

    if (action) query._action = action;
    else if (this.env.action) query._action = this.env.action;
  ...
  this.redirect = function (url, lock) {
    if (lock || lock === null) this.set_busy(true);
  ...
  this.http_response = function (response) {
    ...
    if (response.unlock) this.set_busy(false);
    ...
    switch (response.action) {
      case "mark":
        ...
        if (
          (this.env.action == "show" || this.env.action == "preview") &&
          this.env.last_flag == "SEEN"
        )
      ...
      case "list":
        if (this.task == "mail") {
          ...
          this.enable_command(
            ...
            "select-all",
            "select-none",
            this.env.messagecount > 0
          );
          this.enable_command("expunge", this.env.exists && !is_multifolder);
          this.enable_command(
            "purge",
            this.purge_mailbox_test() && !is_multifolder
          );
  ...
  this.http_error = function (request, status, err, lock, action) {
    ...
    if (request.status && errmsg)
      this.display_message(
        this.get_label("servererror") + " (" + errmsg + ")",
        "error"
      );
    ...
    if (location_url && this.env.action != "compose")
      // don't redirect on compose screen, contents might get lost (#1488926)
  ...
  */
  ...
  this.multi_thread_send_request = function (prop, item) {
    ...
    return postdata
      ? this.http_post(prop.action, postdata)
      : this.http_request(prop.action, query);
  };
  ...
  this.file_dropped = function (e) {
    ...
    var submit_data = function () {
      if (
        ref.env.max_filesize &&
        ref.env.filesizeerror &&
        size > ref.env.max_filesize
      ) {
      ...
      if (
        ref.env.max_filecount &&
        ref.env.filecounterror &&
        numfiles > ref.env.max_filecount
      ) {
      ...
      var multiple = files.length > 1,
        ...
        // jQuery way to escape filename (#1490530)
        content = $("<span>")
          .text(multiple ? ref.get_label("uploadingmany") : files[0].name)
          .html();

      // add to attachments list
      if (
        !ref.add2attachment_list(ts, {
          name: "",
          html: content,
          classname: "uploading",
          complete: false,
    ...
    for (var j = 0, i = 0, f; j <= last && (f = files[i]); i++) {
      ...
      if (
        this.env.filedrop.filter &&
        !f.type.match(new RegExp(this.env.filedrop.filter))
      ) {
  ...
  this.opener = function (deep, filter) {
    ...
    try {
      if (win && !win.closed) {
        ...
        if (
          deep &&
          (!win.rcmail || win.rcmail.env.framed) &&
          win.parent &&
          win.parent.rcmail
        )
          win = win.parent;
        ...
        if (win.rcmail && filter)
          for (i in filter) if (win.rcmail.env[i] != filter[i]) return;
  ...
  this.check_protocol_handler = function (name, elem) {
    ...
    if (!nav || typeof nav.registerProtocolHandler != "function") {
      ...
      var status = nav.isProtocolHandlerRegistered(
        "mailto",
        this.mailto_handler_uri()
      );
      if (status)
  ...
  this.pdf_support_check = function () {
    ...
    if ("ActiveXObject" in window) {
      try {
        if ((plugin = new ActiveXObject("AcroPDF.PDF"))) return 1;
      } catch (e) {}
      try {
        if ((plugin = new ActiveXObject("PDF.PdfCtrl"))) return 1;
      } catch (e) {}
    }
  ...
  this.local_storage_set_item = function (key, data, encrypted) {
    ...
    try {
      ...
      localStorage.setItem(
        this.get_local_storage_prefix() + key,
        JSON.stringify(data)
      );
      return true;
    } catch (e) {
  ...
  this.print_dialog = function () {
    if (bw.safari) setTimeout("window.print()", 10);
    else window.print();
  };
